...
export default function SetSolo() {
    const [gameState, setGameState] = useState<SetGameState>(gameActions.createNewGame());
    ...
    useEffect(() => {
        if (gameState.selectedIndices.length === 3) {
            const selectedCards = gameState.selectedIndices.map(i => gameState.board[i]);
    ...
    return (
        <div className="flex flex-col items-center gap-3 ">
            <Button className="rounded-full bg-transparent border border-white" size="icon" onClick={handleDrawCards}>
